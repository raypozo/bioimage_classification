...
def knn_random(X_l,X_u,Y_l,Y_u,batch_size):
    n=X_u.shape[0]
...
#k_set contains the set of parameter to construct different committees
def knn_query_by_committee(X_l,X_u,Y_l,Y_u,batch_size):
    k_set=[3,5,7]
    c_size=len(k_set)   #This is the size of the Committee
    u_size=X_u.shape[0]
    entropies=np.zeros([1,u_size],dtype=float)
    #Construct different committees with different settings
    models=[]
    for i in range(c_size):
        model=knn_model(X_l,Y_l,k_set[i])
        models.append(model)
    pred_models=[]
...
def knn_uncertainty_sampling(X_l,X_u,Y_l,Y_u,batch_size,k):
    model=knn_model(X_l,Y_l,k)
...
def plot_knn(rdm_accuracy,active_accuracy,rdm_error,active_error,cost,active_method,data_pool):
    ...
    plt.ylabel('Error')
    plt.xlabel('Cost')
    plt.title('KNN with %s, Dataset: %s (Error)'%(enum2[active_method],enum[data_pool]))
    plt.figure(2)
    ...
    plt.ylabel('Accuracy')
    plt.xlabel('Cost')
    plt.title('KNN with %s, Dataset: %s (Accuracy)'%(enum2[active_method],enum[data_pool]))
    plt.show()
    return

def run_knn(train_data,train_labels,test_data,test_labels,budget,active_method,data_pool):
    ...
    for i in range(budget/batch_size):
        if active_method==1:
            active_l_data,active_u_data,active_l_labels,active_u_labels=knn_uncertainty_sampling(active_l_data,active_u_data,active_l_labels,active_u_labels,batch_size,k)
        elif active_method==2:
            active_l_data,active_u_data,active_l_labels,active_u_labels=knn_query_by_committee(active_l_data,active_u_data,active_l_labels,active_u_labels,batch_size)
        else:
            print("Not valid Active Method Selection")
            return

        rdm_l_data,rdm_u_data,rdm_l_labels,rdm_u_labels=knn_random(rdm_l_data,rdm_u_data,rdm_l_labels,rdm_u_labels,batch_size)
        #Train Active-learning and Random model independently
        model_rdm=knn_model(rdm_l_data,rdm_l_labels,k)
        model_active=knn_model(active_l_data,active_l_labels,k)
    ...
    #Plot Results
    plot_knn(rdm_accuracy,active_accuracy,rdm_error,active_error,cost,active_method,data_pool)
